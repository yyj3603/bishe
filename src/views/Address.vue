      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
<!--                使选中的索引和当前循环的索引相同，使选中的地址卡片添加选中的class-->
                <li :key="index" v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkedIndex==index}" @click="checkedIndex=index;selectedAddrId=item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
...
<script>
  ...
  export default {
    data() {
      return {
        limit:3,
        checkedIndex:0,/*默认选中这个地址*/
        selectedAddrId:'',/*被选中的address*/
        addressList:[],
        isMdShow:false,
        addressId:''/*用来删除的addressId*/
      }
    },
    mounted(){
      this.init();
    ...
    methods:{
      ...
      init(){
        axios.get("/users/addressList").then((response)=>{
          ...
          console.log("this.addressList.length的值为"+this.addressList.length)
        });
      },
      /*展开*/
      expand(){
        console.log("触发展开事件后this.addressList.length变成了:"+this.addressList.length)
