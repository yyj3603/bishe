              ...
              <li :key="index" v-for="(item,index) in cartList" >
                ...
                <div class="cart-tab-3">
                  <div class="item-quantity">
                    ...
                    <div class="item-stock item-stock-no">In Stock(有库存)</div>
                  </div>
                </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      init(){
        /*拿出了购物车的所有数据 是不是选中的在html中通过v-if判断checked是否为1*/
        axios.get("/users/cartList").then((response)=>{/*通过 promise链式调用*/
